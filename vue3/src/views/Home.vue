<template>
  <div class="home">
    ...
    <div v-for="(key, index) in Object.keys(algorithm)" :key="index">
      <p>---------------------</p>
      <h3>{{index+1}}.{{ algorithm[key].title }} - {{ algorithm[key].timeComplexity }} - {{ algorithm[key].stability }}</h3>
      <p>
        输出：<span v-for="(val, i) in algorithm[key].arr" :key="i">-{{ val }}-</span>
      </p>
      <div v-if="algorithm[key].describe">
        <p>{{algorithm[key].describe.val}}</p>
        <ul v-if="false">
          <li v-for="(val, i) in algorithm[key].describe.arr" :key="i" >{{val}}</li>
        </ul>
      </div>
...
export default defineComponent({
  setup() {
    const RawData = reactive({
      title: `经典算法`,
      arr: [ 3, 44, 38, 5, 47, 15, 156, 36, 102, 128, 26, 27, 2, 46, 4, 19, 50, 48 ],
      classify: [
        {
          type: `比较类排序`,
          describe: `不通过比较来决定元素间的相对次序，它可以突破基于比较排序的时间下界，以线性时间运行，因此也称为线性时间非比较类排序。`,
          arr: ['交换排序：冒泡、快速', '插入排序：简单插入、希尔', '交换排序：简单选择、堆', '交换排序：二路归并、多路归并']
        },
        {
          type: `非比较类排序`,
